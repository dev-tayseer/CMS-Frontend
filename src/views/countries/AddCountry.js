...
const AddCar = () => {
  ...
  const getCompanyById = async (company_id) => {
    ...
    try {
      const result = await axios.get(`${domain_url}/Country/CountryView?id=${company_id}`, config)
  ...
  const saveDataCompany = async () => {
    ...
    try {
      ...
      if (add['current'] === true) {
        ...
        axios({
          method: "post",
          url: `${domain_url}/Country/CountryView`,
          data: rawdata,
          headers: {
          ...
          .then(function (response) {
            ...
            if (response.data[0]["success"] === 0) {
              ...
              setShowAlert(true)
              if (add['current'] === true) {
                navigate('/countries?success=added')
              } else {
                navigate('/countries?success=edited')
    ...
    <>
      ...
      <div className="theme-content px-0 px-md-5 ms-xl-5 ">
        <div className="container-fluid pe-md-5 ps-md-4 pe-3 ps-3">
          ...
          {(add['current']) ? <NationalitiesHeader headerContract="الدول" headerContract2="الشركات" headerContractObj="إضافة دولة" urlMain="/countries" nonext="false" /> : <NationalitiesHeader headerContract="الدول" headerContract2="الشركات" headerContractObj="تعديل دولة" urlMain="/countries" nonext="false"  />}
          {/* <HomeHeader headerContract="الجنسيات" nonext={false}  /> */}


          {(add['current']) ? <div className='addPageHeader pt-4 ps-0 ms-0'>إضافة دولة</div> : <div className='addPageHeader pt-4 ps-0 ms-0'>تعديل دولة</div>}
          ...
          <Form onSubmit={handleSubmit(onSubmit)}>
              ...
              <Row >
                <Col sm='12' className='mb-4 form-group bmd-form-group'>
                  ...
                  <Controller
                    ...
                    control={control}
                    render={({ field }) => <Input {...field} placeholder='أدخل الدولة  بالعربية' bsSize='lg' className={add['current'] ? 'h-60 f-s-12px f-w-700 bg-F7FAF7' : 'h-60 f-s-12px f-w-700 border-A0A8B3 bg-F9FAFB'} invalid={errors.name_arabic && true} />}
                  />
                ...
                <Col sm='12' className='mb-4 form-group bmd-form-group'>
                  ...
                  <Controller
                    ...
                    control={control}
                    render={({ field }) => <Input {...field} placeholder='أدخل الدولة  بالإنجليزية' bsSize='lg' className={add['current'] ? 'h-60 f-s-12px f-w-700 bg-F7FAF7' : 'h-60 f-s-12px f-w-700 border-A0A8B3 bg-F9FAFB'} invalid={errors.name && true} />}
                  />
            ...
            <div className="row flex-column-reverse flex-sm-row pt-2 mb-2">
              <Col md='6' className='mb-2'>
                <Button className={(nameValidation & name_arabicValidation) ? 'text-center btn-style color-5F605F d-block p-2 w-100 f-w-700 f-s-20px background-FBB827 font-Cairo' : 'text-center btn-style color-BDBFBD d-block p-2 w-100 f-w-700 f-s-20px background-FFF5E0 font-Cairo'} color='white' outline type='submit'> حفظ الدولة  </Button>
