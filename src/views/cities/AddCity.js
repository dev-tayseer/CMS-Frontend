...
const AddCar = () => {
  ...
  const getCompanyById = async (company_id) => {
    ...
    try {
      const result = await axios.get(`${domain_url}/City/CitiesView/?id=${company_id}`, config)
      console.log(result, "edit ..... ")
      if (result.status === 200) {
        const data = result.data.data[0]
        setName(data.city)
        setName_arabic(data.city_arabic)
        setValue('name', data.city)
        setValue('name_arabic', data.city_arabic)
        loadOptionsDB_Country2(data.country.country_id)
  ...
  const loadOptionsDB_Country2 = async (datax) => {
    console.log("again loading ...")
    const token = localStorage.getItem("token")
  ...
  const saveDataCompany = async () => {
    ...
    if (add['current'] === true) {
      var rawdata =
      {
        "data": [
          {
            "city": name,
            "city_arabic": name_arabic,
            "country_id": countryValueId
    ...
    try {
      ...
      if (add['current'] === true) {
        ...
        axios({
          method: "post",
          url: `${domain_url}/City/CitiesView/`,
          data: rawdata,
          headers: {
          ...
          .then(function (response) {
            ...
            if (response.data[0]["success"] === 0) {
              ...
              setShowAlert(true)
              if (add['current'] === true) {
                navigate('/cities?success=added')
              } else {
                navigate('/cities?success=edited')
    ...
    <>
      ...
      <div className="theme-content px-0 px-md-5 ms-xl-5 ">
        <div className="container-fluid pe-md-5 ps-md-4 pe-3 ps-3">
          ...
          {(add['current']) ? <NationalitiesHeader headerContract="المدن" headerContract2="الشركات" headerContractObj="إضافة مدينة" urlMain="/countries" nonext="false" /> : <NationalitiesHeader headerContract="المدن" headerContract2="الشركات" headerContractObj="تعديل مدينة" urlMain="/countries" nonext="false" />}
          {/* <HomeHeader headerContract="الجنسيات" nonext={false}  /> */}


          {(add['current']) ? <div className='addPageHeader pt-4 ps-0 ms-0'>إضافة مدينة</div> : <div className='addPageHeader pt-4 ps-0 ms-0'>تعديل مدينة</div>}
          ...
          <Form onSubmit={handleSubmit(onSubmit)}>
              ...
              <Row >
                <Col sm='12' className='mb-4 form-group bmd-form-group'>
                  ...
                  <Controller
                    ...
                    control={control}
                    render={({ field }) => <Input {...field} placeholder='أدخل المدينة  بالعربية' bsSize='lg' className={add['current'] ? 'h-60 f-s-12px f-w-700 bg-F7FAF7' : 'h-60 f-s-12px f-w-700 border-A0A8B3 bg-F9FAFB'} invalid={errors.name_arabic && true} />}
                  />
                ...
                <Col sm='12' className='mb-4 form-group bmd-form-group'>
                  ...
                  <Controller
                    ...
                    control={control}
                    render={({ field }) => <Input {...field} placeholder='أدخل المدينة  بالإنجليزية' bsSize='lg' className={add['current'] ? 'h-60 f-s-12px f-w-700 bg-F7FAF7' : 'h-60 f-s-12px f-w-700 border-A0A8B3 bg-F9FAFB'} invalid={errors.name && true} />}
                  />
                ...
                <Col sm='4' className='mb-4 form-group bmd-form-group'>
                  <SelectWithValidation1 defaultValue={countryValue} id_name='country' title='الدولة التابع لها' control={control} errors_check={errors.country} loadOptionsDB={countryData} />
                </Col>
            ...
            <div className="row flex-column-reverse flex-sm-row pt-2 mb-2">
              <Col md='6' className='mb-2'>
                <Button className={(nameValidation & name_arabicValidation) ? 'text-center btn-style color-5F605F d-block p-2 w-100 f-w-700 f-s-20px background-FBB827 font-Cairo' : 'text-center btn-style color-BDBFBD d-block p-2 w-100 f-w-700 f-s-20px background-FFF5E0 font-Cairo'} color='white' outline type='submit'> حفظ المدينة  </Button>

              </Col>
              <Col md='6' className='mb-2'>
                <Button href="/countries" className='text-center btn-style background-E3E5E3 color-5F605F d-block p-2 w-100 f-w-700 f-s-20px font-Cairo' color='white' outline >
                  إلغاء
                </Button>
              </Col>
