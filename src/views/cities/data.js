...
export const columns = ((deleteHandler, viewHandler, can_edit_company, can_delete_company) => [
  ...
  {
    name: 'الدوله التابع لها بالعربية',
    minWidth: '300px',
    sortable: row => true,
    cell: row => (
      <p className='table-black-font ms-1 f-s-12px f-w-400  font-Cairo color-5F605F'>
        {row.country != null ?
          row.country.country_name_arabic
          :
          ""
          }
  ...
  {
    name: 'الدوله التابع لها بالانجيليزيه',
    minWidth: '300px',
    sortable: row => true,
    cell: row => (
      <p className='table-black-font ms-1 f-s-12px f-w-400  font-Cairo color-5F605F'>
    ...
    cell: row => {
      return (
        <div className='d-flex justify-content-end w-100'>
          ...
          {can_edit_company && (
            <a target='_blank' className="blue-color-icon blue-color-icon-h ps-3" href={`addCities?id=${row.id}`}><Edit3 size={20} /></a>
          )}
          {can_delete_company && (
