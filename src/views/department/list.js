...
const Company_list = () => {
  ...
  const [can_add_department, setcan_add_department] = useState(false)
  ...
  useEffect(() => {
   ...
   if (success === "added") {
    ...
    setTypeAlert("Success")
    setTitleAlert("تم تحديث بيانات الإدارة بنجاح.")
    setHeadAlert("")
    setShowAlert(true)
   }
  ...
  useEffect(() => {
    if(allPermissions.includes(9)){
      setcan_add_department(true)
    } 
  ...
  return (
    <div className="theme-content px-0 px-md-5 ms-xl-5">
      ...
      <div className="container-fluid pe-md-5 ps-md-5 pe-1 ps-1">
        ...
        <Row className='container-fluid pe-0 ps-4 ps-md-0 mt-2'>
          <Col className=' col-md-6 col-12 '>
          <HomeHeader headerContract="نظام إدارة العقود" headerContractObj="الإدارات" />
          </Col>
          {can_add_department && (

          
            <Col className='col-md-6 col-12 text-md-end mt-sm-0 mt-1 pt-md-5 pt-2  ps-xl-5 ps-3 col-8  col-md-6 pe-2'>
              <Button.Ripple target='_blank' outline className="btn button1 px-5 py-2" href="/addDepartment">
                <span className='color-5F605F f-s-14px font-cairo f-w-700 pe-2' >إضافة إدارة</span>
                <Plus className="ps-5" />
              </Button.Ripple>
            </Col>
          )}
        ...
        <div className='col-md-12 ps-xl-0  pe-xl-0 ps-3 mt-4 d-flex flex-start'>
            <div className='f-w-700 f-s-24px color-5F605F font-cairo '>الإدارات</div>
            <div className='d-none ms-3 color-D99F22 f-s-16px f-w-700 background-FFFDF7 px-4 mt-1 py-1 borderRadius-16px'>{departmentCount} إدارة</div>
        </div>
        </Row>
        ...
        <TableServerSide setTypeAlert={setTypeAlert} setHeadAlert={setHeadAlert} setTitleAlert={setTitleAlert} setShowAlert={setShowAlert} getStart={getStart} getDraw={getDraw} getLength={getLength} setdepartmentCount={setdepartmentCount} setallPermissions={setallPermissions}  />
      </div>
