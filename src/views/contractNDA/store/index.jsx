...
export const getData = createAsyncThunk('datatables/getData', async params => {
  ...
  try {
    // departmentId
    const response = await axios.get(`${domain_url}/Contract/ContractsNDA/?type_of_contract=3&?date_from=${params.from_date}&date_to=${params.to_date}&compid=${params.companyId.toString()}&depid=${params.departmentId.toString()}&status_id=${params.statuesId.toString()}&search[value]=${params['search[value]'].toString()}&start=${((params.start - 1) * params.length).toString()}&length=${params.length.toString()}&draw=1`, config, params)
