...
const Company_list = () => {
  ...
  const [can_add_perm, setcan_add_perm] = useState(false)
  ...
  useEffect(() => {
    ...
    if(allPermissions.includes(23)){
      setcan_add_perm(true)
    } 
  ...
  const EditUser = () => {
    ...
    return (<Modal aria-labelledby="contained-modal-title-vcenter" centered show={showEditUser2} onHide={handleCloseEditUser}>
      ...
      <div className="col-6">
          ...
          { true && (
              <FormComponentRole handleCloseEditUser={handleCloseEditUser} RowId={setSelectedRowId} setTypeAlert={setTypeAlert}  setHeadAlert={setHeadAlert} setTitleAlert={setTitleAlert}  setShowAlert={setShowAlert} reload_table={reload_table} />
            )
          }
    ...
    <div className="theme-content px-0 px-md-5 ms-xl-5">
      ...
      <div className="container-fluid pe-md-5 ps-md-5 pe-1 ps-4">
        ...
        <Row className='container-fluid pe-0 ps-0 mt-2'>
          ...
          <HomeHeader headerContract="نظام إدارة العقود" headerContractObj="الصلاحيات" />
          </Col>

         {can_add_perm && (
            <Col className='col-md-6 col-12  mt-sm-0 mt-1 pt-md-5 pt-2 text-md-end   col-8  col-md-6 ' >
              <Button.Ripple outline className="btn button1 px-5 py-2" onClick={()=>editUserRowId("")} >
                <span className='color-5F605F f-s-14px font-cairo f-w-700 pe-2' >إضافة صلاحية</span>
                <Plus className="ps-5" />
              </Button.Ripple>
            </Col>
          )}
        ...
        <div className='col-md-12 ps-lg-0 ps-3 pe-0 mt-4 d-flex flex-start'>
            <div className='f-w-700 f-s-24px color-5F605F font-cairo '>الصلاحيات</div>
            {/* <div className='ms-3 color-D99F22 f-s-16px f-w-700 background-FFFDF7 px-4 mt-1 py-1 borderRadius-16px'>{admincount} مشرف</div> */}
        </div>
        </Row>
