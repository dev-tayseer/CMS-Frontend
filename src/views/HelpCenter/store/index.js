...
export const getData = createAsyncThunk('datatables/getData', async params => {
  ...
  const response = await axios.get(`${domain_url}/get-contact-us?is_reply=${params.is_reply}&from_date=${params.from_date}&to_date=${params.to_date}&search_key=${params.q.toString()}&start=${((params.page - 1) * params.perPage).toString()}&length=${params.perPage.toString()}&draw=1`, config, params)
  console.log(params)
