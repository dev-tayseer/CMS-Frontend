...
export const getData = createAsyncThunk('datatables/getData', async params => {
  ...
  try {
    ...
    const response = await axios.get(`${domain_url}/Contract/ContractsAuction/?type_of_contract=2&search[value]=${params.q.toString()}&start=${((params.start - 1) * params.length).toString()}&length=${params.length.toString()}&draw=1`, config, params)
    console.log(params)
    console.log("Ahmed")
