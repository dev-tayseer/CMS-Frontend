...
const role = [
  { id:'', value:'All', label:'الكل' },
  { id:'1', value:'admin', label:'مشرف' },
  { id:'2', value:'collector ', label:'مستخدم سحب' },
  { id:'3', value:'yard', label:'موظف استعاده' },
  { id:'4', value:'visitor', label:'جديد' }
]
mock.onGet('/api/select/query_companies').reply(config => {
  if (config.query_companies.length) {
    const queryLowerCase = config.query_companies.toLowerCase()
    return [200, data.filter(i => i.value.toLowerCase().includes(queryLowerCase))]
  }
  return [200, data]
})

mock.onGet('/api/select/query_roles').reply(config => {
  if (config.query_roles.length) {
    const queryLowerCase = config.query_roles.toLowerCase()
    return [200, role.filter(i => i.value.toLowerCase().includes(queryLowerCase))]
  }
  return [200, role]
})


mock.onGet('/api/select/query_type').reply(config => {
  if (config.query_type.length) {
    const queryLowerCase = config.query_type.toLowerCase()
    return [200, data.filter(i => i.value.toLowerCase().includes(queryLowerCase))]
  }
  return [200, data]
})

mock.onGet('/api/select/query_statues').reply(config => {
  if (config.query_statues.length) {
    const queryLowerCase = config.query_statues.toLowerCase()
    return [200, data.filter(i => i.value.toLowerCase().includes(queryLowerCase))]
  }
  return [200, data]
})

mock.onGet('/api/select/query_employees').reply(config => {
  if (config.query_type.length) {
    const queryLowerCase = config.query_type.toLowerCase()
    return [200, data.filter(i => i.value.toLowerCase().includes(queryLowerCase))]
  }
