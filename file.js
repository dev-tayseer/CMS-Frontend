...
const Car_list = () => {
  ...
  const loadOptionsDB_companies = () => {
    ...
    return axios.get(`${domain_url}/get-car-companies`, config, { query_companies }).then(res => {
    ...
    if (filesCertificate.length > 0) {   
        ...
        .then(function (response) {
          ...
          if (response.data.status !== 200) {
            ...
            setTypeAlert("Success")
            setTitleAlert("تم رفع بيانات قائمة المركبة بنجاح.")
            setHeadAlert("")
            setShowAlert(true)
          }
  ...
  const ModalCertificateUpload = () => {
    ...
    return (
      ...
      <UploadWithHtmlBodyV2 modal_animation={modal_animation}  activationStatues2={true} activationStatues={activationStatues}  stateShowDialog={uploadCertificateModal} handleCloseConfirm={handleUploadCertificateClose} handleAccept={handleFileCertificateUpload} title='تأكيد رفع محضر التنفيذ ' classHeader='background-F2F7FF' accept='رفع المستند' content={content} cancel='إلغاء' btn_color='4788C6'  inactive_color='E0EDFF'/>
  ...
  const ModalUploadDoc = () => {
    ...
    const content =  (
      ...
      <div className="row p-0 m-0 pt-3">
        <FileUploaderSingle setModal_animation={setModal_animation} setActivationStatues={setActivationStatues} handleFileUpload={handleFileUpload} handleRemoveAllFiles={handleRemoveAllFiles} files={files} setFiles={setFiles} />
      </div>
    </div>
    )
  ...
  axios({
    url: `${domain_url}/get-cars?start=0&length=500&return_excel=1`, //your url
    method: 'GET',
    responseType: 'blob', // important
    headers:headers
}).then((response) => {
  ...
  return (
    <div className="theme-content px-0 px-md-5">
      ...
      <ConfirmAction stateShowDialog={showConfirmUpload} handleCloseConfirm={handleCloseConfirmUpload} handleAccept={handleFileUpload} title='تأكيد رفع محضر التنفيذ ' classHeader='background-F2F7FF' accept='تأكيد' content='هل انت متأكد ان هذا المستند هو الخاص بهذه المركبة؟' cancel='إلغاء' btn_color='4788C6'/>
      ...
      <div className="container-fluid pe-md-5 ps-md-5 pe-1 ps-1">
            ...
            <Row className='mt-4 row mx-auto'>
              ...
              <Col className='d-flex align-items-center justify-content-sm-start p-0 col-6 col-md-auto'>
                <Button.Ripple outline className="button-transparent  text-center" onClick={() => exportExcelAll()}>
                  ...
                  <span className='align-middle ms-25 ps-3' >تصدير اكسيل</span>
                </Button.Ripple>
              </Col>
            </Row>
          </Col>
        ...
        <TableServerSide showUploadCertificate={showUploadCertificate} setTypeAlert={setTypeAlert} setHeadAlert={setHeadAlert} setTitleAlert={setTitleAlert} setShowAlert={setShowAlert}/>
      </div>
